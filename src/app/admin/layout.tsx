...
import { LayoutDashboard, ShoppingCart, Utensils, Calendar, Shirt, Home, LogOut } from "lucide-react";
...
export default function AdminLayout({
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg)] flex flex-col md:flex-row pt-20">
      {/* Sidebar */}
      <aside className="w-full md:w-64 bg-[var(--color-surface)] border-r border-white/5 md:fixed md:top-20 md:bottom-0 md:left-0 z-30">
        <div className="p-6 flex flex-col justify-between h-full space-y-8">
          <div className="space-y-6">
            <span className="text-[10px] font-mono uppercase tracking-[0.2em] text-white/40 font-semibold block px-3">
              Yönetici Paneli
            </span>
              ...
              {sidebarLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-white/60 hover:text-white hover:bg-white/5 transition-all duration-300"
                  >
          ...
          <div className="pt-6 border-t border-white/5 flex flex-col gap-2">
            <Link
              href="/"
              className="flex items-center gap-3 px-3 py-2 text-xs text-white/40 hover:text-white transition-colors"
            >
              <Home size={14} />
            ...
            <button
              ...
              className="flex items-center gap-3 px-3 py-2 text-xs text-red-400/70 hover:text-red-400 transition-colors w-full text-left cursor-pointer"
            >
              <LogOut size={14} />
