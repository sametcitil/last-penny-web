...
const AVAILABLE_SIZES = ["S", "M", "L", "XL", "Standart"];
...
export default function AdminMerchPage() {
  ...
  const [category, setCategory] = useState<ProductType["category"]>("tshirt");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name || price <= 0 || stock < 0) {
      setFormError("Lütfen geçerli ad, fiyat ve stok değerleri giriniz.");
      return;
    }
    ...
    <div className="space-y-8 text-[var(--color-secondary)]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[var(--color-muted)] mt-1">Giyim ve aksesuar stoklarını düzenleyin.</p>
        </div>
      ...
      {loading && products.length === 0 ? (
        ...
        <div className="text-center py-20 border border-dashed border-[var(--color-border)] rounded-2xl bg-white space-y-3">
          <Shirt size={40} className="text-[var(--color-muted)]/30 mx-auto" />
          <p className="text-[var(--color-muted)] font-semibold">Aradığınız kriterlere uygun ürün bulunamadı.</p>
        </div>
      ) : (
            ...
            <tbody>
              {filteredProducts.map((p) => (
                <tr key={p._id} className="transition-colors">
                  ...
                  <td className="font-mono text-sm font-black text-[var(--color-primary)]">₺{p.price}</td>
                  <td className="text-xs text-[var(--color-secondary)]/80 font-semibold font-mono">
                    {p.sizes && p.sizes.length > 0 ? p.sizes.join(", ") : "—"}
                  </td>
                  <td>
                    <div className="flex items-center gap-1.5 text-xs">
                      <span className={`w-1.5 h-1.5 rounded-full ${p.stock > 0 ? "bg-green-600" : "bg-red-600"}`} />
                      <span className="font-bold font-mono text-[var(--color-secondary)]/80">{p.stock} adet</span>
                    </div>
                  </td>
                  <td>
            ...
            >
              ...
              <div>
                <h3 className="font-bold text-lg font-[family-name:var(--font-playfair)]">
                  {editingProduct ? "Ürünü Düzenle" : "Yeni Merch Ürünü Ekle"}
                </h3>
                <p className="text-xs text-[var(--color-muted)]">Mağaza ürün detaylarını doldurun.</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-mono uppercase tracking-wider text-[var(--color-muted)] font-semibold">Ürün Adı</label>
                  <input
                    type="text"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder="Örn: Last Penny Classic Tee"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <select
                      value={category}
                      onChange={(e) => setCategory(e.target.value as ProductType["category"])}
                    ...
                    >
                      ...
                      <option value="hoodie">Sweatshirtler</option>
                      <option value="cap">Şapkalar</option>
                      <option value="accessory">Aksesuarlar</option>
                    </select>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-mono uppercase tracking-wider text-[var(--color-muted)] font-semibold">Stok Miktarı</label>
                    <input
                      type="number"
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    placeholder="Kumaş kalitesi, kalıp bilgisi, tasarım detayları..."
                    rows={3}
                    className="w-full bg-[var(--color-surface-hover)] border border-[var(--color-border)] rounded-xl px-4 py-2.5 text-sm transition-all focus:bg-white"
                  />
                </div>
                ...
                {/* Sizes selection */}
                <div className="space-y-1.5">
                  <label className="text-xs font-mono uppercase tracking-wider text-[var(--color-muted)] font-semibold block">Beden Seçenekleri</label>
                    ...
                    {AVAILABLE_SIZES.map((size) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-3.5 py-2 text-xs rounded-xl font-mono border transition-all cursor-pointer ${
                            isSelected
                              ? "border-[var(--color-primary)] bg-[var(--color-primary)]/5 text-[var(--color-primary)] font-bold"
                              : "border-[var(--color-border)] bg-[var(--color-surface-hover)] text-stone-500 hover:text-stone-700"
                          }`}
                        >
                          {size}
                        </button>
